...
A typical `vetiver` workflow within Posit Team consists of converting a model to a *vetiver model*, saving it to Posit Connect as a `pin`, and then serving it as an API. This workflow is shown below in both Python and R. Be sure to substitute "your_name" with your Posit Connect username:
...
Now that we have a model *saved* to Posit Connect, we can use `vetiver` to serve it as an API. By default, `vetiver` will use FastAPI for python models, and `plumber` for R models. Creating APIs is a cinch with `vetiver`, and we can do it with one line of code:

::: {.panel-tabset group="language"}
## Python

``` python
vetiver.deploy_rsconnect(board, "you_name/cars_linear")
```
...
vetiver_deploy_rsconnect(board, "your_name/cars_linear")
```
:::
